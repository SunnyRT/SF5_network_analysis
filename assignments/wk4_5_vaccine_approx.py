...
if __name__ == "__main__":
    ...
    # FIXME: need to average over vaccination set V
    ...
    for idx, lambda_ in enumerate(lam_ary):
        s_vi = compute_s_prob_adjls(edge_ls_vi, lambda_, n*v)
        s_vj = compute_s_prob_adjls(edge_ls_vj, lambda_, n*v)
        s_ary_vi[idx] = np.mean(s_vi)
        s_ary_vj[idx] = np.mean(s_vj)

    Nr_ary_vi = (1 - s_ary_vi)*n*v # output shape (lam_n,)
    Nr_ary_vj = (1 - s_ary_vj)*n*v # output shape (lam_n,)
