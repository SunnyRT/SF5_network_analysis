...
def time_compare():
    ...
    for i, n in enumerate(n_ary):
        ...
        t_ary_3[i] = timeit.timeit(lambda: rm_graph_gen3(n, p, True), number=100)
    print("time for naive sampling algorithm:")
    print(t_ary_1)
    print(f"time for 2-stage algorithm:")
    print(t_ary_2)
    print(f"time for inbuilt algorithm:")
    print(t_ary_3)
    ...
    plt.loglog(n_ary, t_ary_3, "r", label="inbuilt sampling")
    plt.loglog(n_ary, np.exp(t_2_fit(np.log(n_ary))), "--r", label=f"inbuilt slope: {coeff_3[0]:.2f}")
